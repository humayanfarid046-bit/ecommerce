      ...
      >
        ...
        <button
          ...
          className={cn(
            ...
            listening && "animate-pulse bg-[#2563eb]/12 text-[#2563eb]"
          )}
        ...
        <button
          ...
          className={cn(
            ...
            variant === "flipkart"
              ? "hover:text-[#2563eb] dark:hover:bg-slate-800 dark:hover:text-[#7eb3ff]"
        ...
        <button
          ...
          className={cn(
            ...
            variant === "flipkart"
              ...
              : "rounded-xl bg-[#2563eb] hover:bg-[#1d4ed8]"
          )}
        >
          {t("submit")}
        </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            exit={{ opacity: 0, y: -6 }}
            className="absolute left-0 right-0 top-full z-40 mt-2 max-h-80 overflow-auto rounded-[12px] border border-slate-200/90 bg-white/95 py-2 shadow-[0_16px_48px_rgba(37,99,235,0.14)] backdrop-blur-xl dark:border-slate-600 dark:bg-slate-900/95 dark:shadow-black/30"
            role="listbox"
          >
            {suggestions.map((p) => (
              <li key={p.id} role="option" aria-selected={false}>
                <Link
                  href={`/product/${p.id}`}
                  className="flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition hover:bg-[#2563eb]/8 dark:hover:bg-[#2563eb]/10"
                  onClick={() => setOpen(false)}
                >
                  <span className="line-clamp-1 text-slate-900 dark:text-slate-100">{p.title}</span>
                  <span className="ml-auto text-xs font-semibold text-[#2563eb] dark:text-[#7eb3ff]">
                    {p.brand}
                  </span>
                </Link>
              </li>
